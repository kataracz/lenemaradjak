...
export function PodcastsWidget({ publisherIds }: { publisherIds: string[] }) {
        ...
        if (failedFeeds.length > 0) {
          setError(
            failedFeeds.length === 1
              ...
              : `${failedFeeds.length} RSS csatorna nem töltődött be. A sikeres epizódok továbbra is megjelennek.`,
          );
        }
  ...
  return (
    <DashboardCard
      ...
      actions={
        <Button variant="outline" size="sm" onClick={() => void loadPodcasts()}>
          Frissítés
        </Button>
      }
    >
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <p className="line-clamp-2 text-sm text-muted-foreground">
                    {item.description ?? "No episode summary available."}
                  </p>
                </article>
              ))}
            </div>
